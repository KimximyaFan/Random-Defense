...
public class G1_2357
{
    public static void main (String[] args) throws IOException
    {
        ...
        for (int i=0; i<M; i++)
        {
            ...
            sb.append(seg.Range_Operation(a, b)).append(" ").append(seg.Range_Operation2(a, b)).append("\n");
        }
    ...
    static class Segment_Tree
    {
        ...
        public Segment_Tree(long[] num) 
        {
            ...
            segment_tree2 = new long[4*this.num.length + 1];

            Make_Segement_Tree(1, 0, num.length-1);
            Make_Segement_Tree2(1, 0, num.length-1);
        }
        ...
        public long Range_Operation2(int num_start, int num_end)
        {
            return Get_Range_Result2(1, 0, num.length-1, num_start, num_end);
        }
        ...
        private static long Operator2(long left, long right)
        {
        ...
        private long Make_Segement_Tree2(int node, int start, int end)
        {
            if (start == end)
                return segment_tree2[node] = num[start];

            int mid = (start + end) >> 1;
            long left_result = Make_Segement_Tree2(node << 1, start, mid);
            long right_result = Make_Segement_Tree2(node << 1 | 1, mid+1, end);

            return segment_tree2[node] = Operator2(left_result, right_result);
        }
        ...
        private long Get_Range_Result2(int node, int seg_start, int seg_end, int num_start, int num_end)
        {
            if (num_end < seg_start || seg_end < num_start)
                return Identity2();

            if ( num_start <= seg_start && seg_end <= num_end )
                return segment_tree2[node];

            int mid = (seg_start+seg_end) >> 1;
            long left_result = Get_Range_Result2(node << 1, seg_start, mid, num_start, num_end);
            long right_result = Get_Range_Result2(node << 1 | 1, mid+1, seg_end, num_start, num_end);

            return Operator2(left_result, right_result);
        }
